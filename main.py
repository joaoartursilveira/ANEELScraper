...
def relacao_arquivos_novos(df_padrao):
    ...
    df_novos = pd.merge(df_padrao, df_banco, on=list(
        df_banco.columns), how='left', indicator=True)
...
def atualizar_dados(dn):
    ...
    while True:
        resposta = input(
            'Deseja atualizar o banco e realizar o download dos arquivos printados acima? [y/n] ')
        if resposta == 'y':
...
def main():
    ...
    if len(df_novos) == 0:
        ...
        print(
            f'Último processo mapeado: {df_aneel_padrao.iloc[0, :].values.tolist()}')
        return None
